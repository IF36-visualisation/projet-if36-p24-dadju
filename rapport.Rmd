...
-Est-ce qu'il y'a une différence de la durée des films dans chaque pays?
...
ggplot(data = data_filtered_1, aes(x = budget, y = popularity)) +
  ...
  labs(x = "Budget du film (en millions de dollars)", y = "Popularité", title = "Popularité en fonction du budget des films") +
  theme_gray() 
...
Ici, on va s'intéresser à la relation qui existe entre la notre d'un film et le ou les acteurs du film.  Quel est le lien entre les acteurs et la note ou la popularité du film ?

Cette analyse vise à déterminer la relation entre les acteurs et la popularité ou la note attribuée à un film. Nous cherchons à savoir si la présence d'un acteur influence la note ou la popularité du film. La note du film est une variable quantitative continue, et l'acteur est une donnée qualitative nominale de ce fait ,
...
De plus, les films avec des acteurs célèbres bénéficient généralement d'une meilleure couverture médiatique et de campagnes de marketing plus agressives. Cette visibilité accrue attire un public plus large et peut également contribuer à des notes plus élevées pour le film.
...
Des screenshots des deux pages du dashboard ont été réalisés pour être montrés dans ce rapport, mais nous détaillerons ensuite les différents graphiques en les rendant accessibles directement depuis ce rapport. L'utilisateur peu se rendre directement dans le dossier shiny pour manipuler le dashboard s'il le souhaite.
...
```{r relation entre le budget et la durée d'un film }
movies <- data
...
Le graphe obtenu montre qu'il existe une corrélation positive entre le budget et la durée d'un film car la durée d'un film augmente proportionnellementavec son budget. Aussi certains films à faibles budgets sont des films dont la durée est  grande par rapport aux autres est ce pour dire que les films à faible budget dure plus qu'un film à grand budget?cette diférence de durée est ce due au fait que les film ne sont pas du meme genre ou est ce du à des contraintes cinématographiques?Ce pendant on remarque que certains films ont un budget de 0$ et une durée de 0 minutes ce qui n'est pas normal
...
# Fonction pour extraire les codes ISO des pays de production
extract_iso_codes <- function(json_string) {
# Parse the JSON string
  parsed_json <- fromJSON(json_string, simplifyDataFrame = FALSE)
  
# Extract the ISO codes
  iso_codes <- sapply(parsed_json, function(x) x$iso_3166_1)
  
  return(iso_codes)
}

# Extraire les pays de production et la durée des films
movies <- data
  mutate(production_countries = lapply(production_countries, extract_iso_codes))

# Ajouter les pays modifiés à un nouveau dataframe
movies_countries <- data.frame(matrix(ncol = 2, nrow = 0))
colnames(movies_countries) <- c("country", "runtime")

for (i in 1:nrow(movies)) {
  if (!is.null(movies$production_countries[[i]]) && length(movies$production_countries[[i]]) > 0) {
    for (country in movies$production_countries[[i]]) {
      movies_countries <- rbind(movies_countries, data.frame(country = country, runtime = movies$runtime[i]))
    }
  }
}

# Filtrer les pays qui produisent un nombre significatif de films (au moins 10 films)
country_counts <- movies_countries %>%
  group_by(country) %>%
  summarise(count = n()) %>%
  filter(count >= 10)

movies_filtered <- movies_countries %>%
  filter(country %in% country_counts$country)

# Calculer la durée moyenne des films par pays
avg_runtime_by_country <- movies_filtered %>%
  group_by(country) %>%
  summarise(avg_runtime = mean(runtime, na.rm = TRUE))

# Créer le barplot pour la durée moyenne des films par pays
ggplot(avg_runtime_by_country, aes(x = reorder(country, -avg_runtime), y = avg_runtime, fill = country)) +
  geom_bar(stat = "identity") +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
# etudions la distribution de cette visualisation avec un boxplot

# Créer le boxplot pour la distribution des durées des films par pays
plot_interactif_1 <- ggplot(movies_filtered, aes(x = reorder(country, -runtime), y = runtime, fill = country)) +
  geom_boxplot() +
...
Dans une seconde partie, nous allons chercher à connaitre l’évolution de la popularité des films en fonction des années, car comprendre cette évolution au fil des ans peut fournir des informations précieuses sur les tendances de consommation de médias, les préférences du public et les changements culturels.
...
Pour l'affichage du graphique, nous n'avons considéré la popularité qu'entre 0 et 250 car ce n'est que dans cet intervalle que les points sont representés. Aussi, cette décision a été prise afin d'obtenir une meilleure appréciation et lisibilité de la popularité moyenne des films.
...
Même si la prépondérance des Etats-Unis est notable, il convient de souligner qu'à partir des années 2000, les pays producteurs se diversifient.
...
```{r prétraitement et création d’une fonction qui réalise le graphique du top 3 des genres de films par compagnie de production tous les 5 ans }
...
Après ces différentes comparaisons malgré différents pays de production, nous comprenons donc que ces compagnies cherchent non seulement à élargir leur type de genre de films mais aussi se conformer à la tendance du marché. Etant donné que la plupart produisent les mêmes genres de films à un moment donné et ne cherchent qu’à générer et à augmenter leurs profits.



### Analyse des mots employés dans les films

Dans cette partie, nous allons chercher à savoir si certains mots-clés ressortent dans les films d'une compagnie de production particulière ou pour un réalisateur donné, et peut-être déterminer s'il existe un lien avec les pays de production.
