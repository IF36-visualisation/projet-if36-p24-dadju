...
Ici la popularité a été filtrée pour ne sélectionner que celle inférieure à 150. En effet, seules 19 valeurs sont au dessus de cette limite que nous nous fixons. La popularité moyenne est de `r round(mean(data$popularity[data$popularity < 150]), 1)`.
...
Les données étant biaisées, il est judicieux d'utiliser la durée moyenne des films pour avoir un meilleur graphique.
...
Pour réaliser une évolution, nous avons utilisé un geom_line(). Les variables utilisées, telles que la popularité et la date de sortie, sont des variables quantitatives continues. 
...
Pour réaliser une comparaison, nous avons utilisé un geom_bar(). Les variables utilisées, telles que production_countries et release_date, sont respectivement des variables nominales et quantitatives continues. 
