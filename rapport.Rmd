...
```{Filtrer les années jusqu'à l'année 2017 car le dataset ne comporte plus d'informations après celle-ci }
average_popularity_by_year <- average_popularity_by_year %>% filter(year < 2017)
```

```{Créer un graphique de la popularité moyenne des films par année}
...
```{Charger les données de films à partir du fichier CSV}
data <- read.csv ("movies.csv")
```
...
```{Changer ou modifier la durée des tranches et creation du graphique de la distribution de la popularite des films }
intervalles <- seq(1900, 2020, by = 5)
...
```{Fonction pour nettoyer la colonne en ne gardant que les lettres majuscules}
...
```{Séparer la liste de façon propre }
...
```{Convertir et extraire l'année de la date de sortie }
movies_data$release_date <- as.Date(movies_data$release_date)
movies_data$year <- format(movies_data$release_date, "%Y")
year <- movies_data$year
```

```{Compter le nombre de films réalisés par pays de production et par année }
...
```{Ne garder que ceux qui, pour une année donnée, on fait plus de 5 films }
films_par_pays_par_annee <- films_par_pays_par_annee %>%
  group_by(production_countries, year) %>%
  filter(sum(nombre_de_films) > 5)
```

```{creation du graphique du nombre de films réalisés par pays et par année }
