...
Vérifions si la durée d'un film varie en fonction du genre. Pour cela, créons un diagramme en barres.
...
company_data <- data[data$production_companies == company_name, ]
   ...
   corpus <- Corpus(VectorSource(keywords))
    ...
    mtext(paste("Nuage des mots-clés de", company_name), side = 3, line = 4, font = 1, cex = 1.5)
  }
```
...
```{r Fonction pour tracer un wordcloud des mots-clés pour un pays producteur donne }
  top_keywords_by_countries <- function(countries_name, data) {
    ...
    wordcloud(words = top_keywords$word, freq = top_keywords$freq, random.order = FALSE, rot.per = 0.3, 
              scale = c(4, 0.5), max.words = 100, colors = brewer.pal(8, "Dark2"))
    mtext(paste("Nuage des mots-clés de", countries_name), side = 3, line = 4, font = 1, cex = 1.5)
  }
```
...
- Etude de l'évolution de la popularité des films dans le temps 

- Etude sur les pays ayant réalisé le plus de films en fonction des années

- Etude sur les maisons de production ayant réalisées le plus de films en fonction des années

- Etude de la variation de genres de films des maisons de production au fil du temps

- Etude des mots-clés ressortant des films de telle ou telle compagnie de production ou pour tel réalisateur et savoir si  cela a un lien avec les pays de production

- Contribution à la rédaction de l'introduction et de la conclusion
